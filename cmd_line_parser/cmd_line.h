...
// EMWHBR: Moved the entire #include-section to the beginning of this file.
//#include <iostream> // you may need this
#include <map>
#include <string>
#include <vector>

// EMWHBR: We are NOT using MFC and Windows, we are using Linux.
////#ifdef __AFX_H__
//// if we're using MFC, use CStrings 
////#define StringType CString
////#else
//// if we're not using MFC, use STL strings
#define StringType std::string   // EMWHBR: See comment of namespacing below.
///#endif

// tell the compiler to shut up
//#pragma warning(disable:4786)  // EMWHBR: We don't want this

// EMWHBR: We don't want any impicit use of namespace std.
//         Use explicit use of std:: in ALL code below.
//using namespace std; 
...
struct CCmdParam
{
    ...
    std::vector<std::string> m_strings; // EMWHBR: To avoid compiler error
};
