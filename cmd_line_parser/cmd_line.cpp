...
//#include "CmdLine.h"
#include "cmd_line.h"  // EMWHBR: Changed file name
//#include "crtdbg.h"  // EMWHBR: Removed this Microsoft stuff.
#include <cstring>     // EMWHBR: Needed for using the strlen function.
...
StringType CCmdLine::GetArgument(const char *pSwitch, int iIdx)
{
   if (HasSwitch(pSwitch))
   {
      ...
      theIterator = find(pSwitch);
	   if (theIterator!=end())
      {
	  if ((*theIterator).second.m_strings.size() > (unsigned)iIdx) // EMWHBR: Casting to unsigned.
         {
