...
joy_exception::joy_exception(const char *file,
			     int line,
			     const char *pretty_function,
			     JOY_ERROR_SOURCE source,
			     long code,
			     const char *info_format, ...)
{
  ...
  m_thread_id  = joy_get_my_thread_id();
  m_process_id = joy_get_my_pid();
}

////////////////////////////////////////////////////////////////

joy_exception::~joy_exception(void) throw()
{
}

////////////////////////////////////////////////////////////////

string joy_exception::get_function(void)
{
...
void joy_exception::get_stack_frames(STACK_FRAMES &frames)
{
  bzero(&frames, sizeof(frames));

  for (int i=0; i < m_nr_frames; i++) {
    frames.frames[i] = (uint64_t)m_stack_frames[i];
  }
...
const char* joy_exception::what() const throw()
{
  ...
  char buffer[18];
  for (int i=0; i < m_nr_frames; i++) {
    sprintf(buffer, "0x%016lx", (uint64_t)m_stack_frames[i]);
...
string joy_exception::get_class_method(const string pretty_function)
{
